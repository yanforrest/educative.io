class PartitionSet {
   ...
   private int canPartitionRecursive(Integer[][]dp, int[] num, int currentIndex, int sum1, int sum2) {
      ...
      if(dp[currentIndex][Math.abs(sum1 - sum2)] != null) {
         System.out.println("hit memorization");
         return dp[currentIndex][Math.abs(sum1 - sum2)];
      }
      ...
      dp[currentIndex][Math.abs(sum1 - sum2)]= Math.min(diff1, diff2);
